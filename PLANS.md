...
- `M20` is done. The full reminders CRUD loop is live in the admin UI.
- Next concrete code milestone: `M21` — surface reminders on the Display/kiosk view. Show a narrow "upcoming reminders" strip at the top of the display that lists any reminders due within the next 30 minutes, auto-refreshing on the same poll cycle as the agenda. This closes the loop for family members who use the kiosk rather than the admin panel.
