...
- `M21` is done. The full Notifications + Display loop is wired: admin creates reminders, worker fires them, kiosk shows upcoming reminders in real time.
- Next concrete code milestone: `M22` — harden the Display/kiosk view for continuous use. Candidates: (a) add a "last refreshed" timestamp to the display footer so owners can confirm the kiosk is live, (b) auto-reload the page if the token is invalid or the projection fails for N consecutive cycles, (c) display mode CSS polish for large-screen readability. Start with (a) as it is the smallest and most observable.
