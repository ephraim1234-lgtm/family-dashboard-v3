...
- `M19` is done. Notifications module is wired into API, Worker, and frontend proxy.
- Next concrete code milestone: `M20` — surface reminders in the frontend UI. Narrowest slice: a reminder management panel on the event detail view (list existing reminders, create with minutes-before input, delete). No push/toast layer yet — pure CRUD to validate the API round-trip before adding real-time delivery.
